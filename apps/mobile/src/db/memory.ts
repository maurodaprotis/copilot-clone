...
export function createMemoryDb(): LocalDb & { _tables: Record<string, Map<string, Row>> } {
  ...
  async function runAsync(sql: string, ...p: SqlValue[]): Promise<unknown> {
    ...
    if (s.startsWith("UPDATE transactions SET review_status =")) {
      // migrate: UPDATE ... WHERE review_status = 'pending' (no params)
  ...
  async function getAllAsync<T>(sql: string, ..._p: SqlValue[]): Promise<T[]> {
    ...
    if (s.includes("review_status IN ('needs_review', 'pending')") || s.includes("review_status = 'needs_review'") || s.includes("review_status = 'pending'")) {
